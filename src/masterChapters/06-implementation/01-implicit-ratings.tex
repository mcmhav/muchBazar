...
using a linear penalization function (Figure~\ref{dist-recentness-linear}
and~\ref{dist-count-linear} respectivly) with the ratings generated on the
same features with a logistic penalization function
(Figure~\ref{dist-recentness-sigmoid} and~\ref{dist-count-sigmoid}). This
yields the following distribution of ratings with the average $3.30$ and median
at $3.63$, where the distributions overlayed with the kernel density function,
showing the trend of histogram:
