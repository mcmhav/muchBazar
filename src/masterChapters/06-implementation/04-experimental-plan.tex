...
Figure \ref{implicitRatingEvaluation} shows the process of evaluating a set of generated ratings. 
...
However, \emph{million dollar question} is: how to do we evaluate a set of ratings? Evaluating and comparing
a set of ratings is not something you often encounter in the literature.
\marginpar{TODO: Are there any articles on something similar?}
Initial theories on how it can be done:
...
Suitable metrics: AUC, MAP, Recommendation - Summarize (Bought, liked, clicked) for both methods

\subsubsection{Implicit Ratings vs. Binary Preference}
...
\begin{itemize}
\item Recommendation methods that can be used for comparison that can incorporate both types of feedback
\item 
\end{itemize}

There is also not an abundance of methods developed for implicit ratings.
Most are focused around explicit ratings and binary ratings.
...
\subsubsection{A Comparison of Implicit Rating Mapping Functions}
