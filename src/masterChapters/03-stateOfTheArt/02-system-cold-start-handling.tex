...
\marginpar{This subset requires some polishing...}
...
into at a later point when more social functionality is in place, to further enhance
...
being scalability challenges, the added data collection needed and the added complexity to the system.
...
find \emph{controversial} items which users either tend
...
all these items. However, their method shows some promise as a selection criteria
for items to ask the user to rate for an interview process.
...
experimented with different datasets and evaluation measures. We therefore opted to do qualitative assessment of the methods in question. It is worth nothing that qualitative methods are less about attempting to prove something than about to understand something. Our ordinal rating scale spans from \ding{55} meaning that the results are bad/not optimal, \checkmark means that the results appear to be satisfactory or simply better than the solutions marked with \ding{55} and finally \checkmark\checkmark means that the results appear to be excellent or very good. The assessment will factor in the following properties:

\begin{itemize}
    \item Accuracy: How does it affect the systems overall performance?
    \item Cold-start performance: How well does the method handle the cold-start related problems?
    \item User-effort: How much user involvement is required before the system can start producing accurate
    recommendations?
\end{itemize}

The following table shows our final qualitative assessment of the different cold-start solutions:

\marginpar{Remove accuracy column? (Not informative)}

\begin{table}[H]
    \centering
    \begin{tabular}{*{4}l}
    \toprule
    Method					& Accuracy 				& Cold-start performance  & User-effort \\ 			\midrule
    Trust-aware RS 			& \checkmark			& \checkmark			  &	\ding{55}				\\ 
    Filterbots 				& \checkmark		   	& \checkmark			  &	\checkmark\checkmark	\\ 
    Seed users 				& \checkmark			& \checkmark			  &	\ding{55}				\\ 
    Intelligent selection 	& 		   				& \checkmark			  &	\checkmark				\\
    Hybrid Methods 			& \checkmark\checkmark	& \checkmark \checkmark	  &	\checkmark\checkmark	\\
    \bottomrule
    \end{tabular}
...
Intelligent selection have not been given an accuracy score as it is not a recommender system in itself. Trust-aware recommender systems get a \ding{55} in user effort as it is hard to find users to connect to,
this problem could however be mitigated by implementing a system like the one described by
Victor et al. \cite{Victor2008} to recommend users to connect to. Seed users also get a \ding{55} in
user-effort, mainly due to its solution to the new item problem where all representative users
must rate an item for it to be recommendable. Filterbots and the Hybrid methods both get two checkmarks for requiring no extra user involvement. Intelligent selection could pretty seamlessly be integrated into the solution using e.g. a simple \emph{hot or not} interface, requiring minimal user effort.
We would like to argue that having the option to incrementally update the model is an important feature which could further improve cold start performance, as it instantly can incorporate the ratings given
by new users and to new items. The hybrid methods therefore gets (++) for their cold-start performance.
From the above table we believe that Filterbots, Intelligent selection and Hybrid Methods, in particular RBLF and Matchbox
could provide us with a good solution to the cold-start problem. However, the hybrid methods are both dependent on good user and item features.
