...
For SoBazaar there are todo sessions in total and todo purchases, so the conversion rate is $todo = \frac{todo}{todo}$, which is quite high compared to the conversion rate in women fashion from figure~\ref{fig:conversionRateFashion}.

This is mainly because it is not possible to do a purchase in the SoBazaar application, the user is instead redirected to the retailer where the currently watched item is sold, and the user can purchase the item there.
This will naturally lead to some non completed purchases, but
