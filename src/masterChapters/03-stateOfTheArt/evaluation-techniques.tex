...
in an offline environment. Various names and abbrivations will be explained as
...
\subsubsection{Evaluation metrics: predictive accuracy vs. relevance}
...
% TODO: Thomas, a good cite for this?
...
% TODO: Introduce the upcoming definitions.
...
% TODO: We are repeating ourselves.
...
% TODO: I think we can delete this section, or move it somewhere else or
% _ensure_ we are not overlapping with any other sections (either above or in
% the future of the thesis)
...
The rank based approach can be considered more suited to test recommendation
...
cannot be produced, other ways of measuring the preference must be used.
Ranking the predicted list and scoring this list based on the actual user
preferences can for instance be utilized to evaluate the system.
The actual user preferences are a list of binary preferences for the items.
This list is often possible to construct from implicit feedback, but will seldom
be able to capture the complete set of preferences of the users, since implicit
feedback is in most cases positive only.
Some approaches to handle measuring a recommender system with implicit feedback
are: Li et al.~\cite{deLace2010} who used $MPR$ and $MAP$ to calculate their
performance,  Pan et al.~\cite{Pan:2013:GGP:2540128.2540516} who used a set of
top-k evaluation metrics: precision, recall, F1, nDCG, ROC and 1-call, Sindhwani
et al.~\cite{Sindhwani:2010:OMC:1933307.1934641} who used $ROC$ curve and
precision-recall curve to evaluate their experiments and Pan et al.~\cite{pan2008} who used $MAP$ and Half-life Utility.

$nDCG$, $MAP$ and Half-life utility has a similar setup.  They all have a
decaying factor, and rewards the system in different ways for how early in the ranked list a relevant recommended item is.

On the other hand metrics as precision alone has no decaying factor, and will
not penalize a system with uninteresting items suggested earlier in a ranked
list.
...
% TODO: Less 'synsing' and refer to what we actually use.
