...
\subsubsection{Fashion in E-commerce}
    ...
    \begin{figure}[H]
        ...
        Paid and Organic are search engine origins, and makes up of 53\% of traffic origin in e-commerce fashion.
    ...
    \begin{figure}[H]
        ...
        \centering
        \caption[Conversion Rate in Fashion Retail]{This figure is taken from~\cite{Jorij2012} and shows where the conversion rate in the fashion domain.
        ...
        The average conversion rate was around 3\% in 2013~\cite{nielsen2013}.
        }
    \end{figure}
...
\label{par:myntra}
    ...
    The web page a set of recommendation approaches to inform the user of what they might like, and increase the user's awareness of different kinds of items. Such as, similar item and most popular.
    \begin{figure}[H]
        \centering
...
\label{par:flink}
    ...
    \end{table}
\marginpar{todo, might be more, but can't explore the application since it is iOS 7 required :P}
% paragraph flink (end)
...
\label{par:motilo}
    ...
    Motilo gives the user the ability to put together item sets though dragging and dropping the items into a "fashion dilemma", or simply like items.
...
\label{par:ustrendy}
    ...
    When the user is viewing an item, UsTrendy displays other items the user might like, which have common traits as with the on the user is currently watching.
...
\label{par:polyvore}
    ...
    When accessing an item the user is shown similar items to the one which is currently watched.
...
\paragraph{Ellos~\cite{Ellos}} % (fold)
\label{par:ellos}
...
\paragraph{Competitors Recommendation Overview} % (fold)
\label{par:competitors_recommendation_overview}
    ...
    There where no indications of the "want list" being used directly to some personalized recommendations and neither was it with the following of other users.
...
% % user sell application, interesting?
% \paragraph{Threadflip} % (fold)
% \label{par:threadflip}
% % paragraph threadflip (end)
...
% \label{par:fancy}
%     "Fancy is the place for you to discover and buy amazing things curated by our global community." - About Fancy~\cite{fancy}
...
%     \subparagraph{Strengths} % (fold)
%         \begin{itemize}
%             \item "Fancy" or like items and earn credits
%             \item Shopping chart in application
%             \item Upload own items
%             \item Suggest users to follow based on what the user likes
%             \item High novelty on the suggested items
%         \end{itemize}
    % subparagraph strengths (end)
    % \subparagraph{Weaknesses} % (fold)
    %     \begin{itemize}
    %         \item Limited personalized recommendation
    %     \end{itemize}
    % subparagraph weaknesses (end)
% paragraph fancy (end)
\subsubsection{Other Fashion Recommender Systems}
...
\paragraph{Scenario-Oriented Recommendation}
    ...
    Ying et al.~\cite{Ying2011} made a recommender based on the a similar concept, namely, what to wear in different situations.
    ...
    To recommend, the system uses the user profile of the user, which describes the interests of the user and the information give by the user about the different items in his or hers wardrobe.

\paragraph{Photograph Recommendation Integrated with Occasion} % (fold)
\label{par:photograph_recommendation_integrated_with_occasion}
    Liu et al.~\cite{Liu:2012:HMC:2393347.2393433} also made a similar recommender.
    Their
