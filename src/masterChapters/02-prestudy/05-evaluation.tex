...
\begin{equation}
	HL_u = \sum_{i}{\frac{max((R_{u,i}-d_i),0)}{2^{\frac{i-1}{\alpha-1}}}}
\end{equation}
$d_i$ is the default rating of item $i$ and $R_{u,i}$ is the rating of user $u$ on item $i$
...
The reason for a user not to access an item is not necessarily grounded in thislike, but could simply be an overlook.
...
Two ways of measuring the system is through ranking~\ref{para:rank_based} and usage prediction~\ref{para:measuring_usage}.
