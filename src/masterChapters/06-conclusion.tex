...
to construct a scoring scheme. Then, for every event exhibited by a user on an
...
Our third component is the system that takes ratings as input and outputs
recommendations.  We tested a wide array of different recommender systems on
...
method for implicit feedback, to be a good fit for our implicit ratings.

% Fourth component: evaluation.
Our fourth component we came up with an set of evaluation metrics to measure
the overall quality of the recommender systems. Evaluating recommender systems
using implicit feedback differs from traditional recommender system evaluation,
simply using RMSE was not enough. We determined that a combination of accuracy
and rank-accuracy metrics was the best fit for our top-20 recommendation task.
AUC measures the overall quality of the recommender system while $MAP@20$ and
our own unique event specific measurements measure the systems ability to rank
...
wants and increase the ranking quality. The improvements over the non-personalized
most popular recommender was a 237\% increase in recall of wanted items
and an increase from 0\% recall of purchased items to 8.33\% for
ordering. For the random split the results were less conclusive.
Here the blend scored an AUC that was 4.3\% higher than the best binary method
and was able to recall both more wanted items and purchases. However, user-based
kNN managed to achieve a higher $MAP@20$ scores when considering wanted and purchased
items. Making our results inconclusive. Our results give some indication that recentness
is an actual factor in the fashion domain. We also found prize to consistently give
better results than popularity, which might give some indication that prize
is a more important factor. Due to the limited size of our dataset we
do not wish to draw any conclusions as our results should be verified
through online experiments.
...
recommendations based on user behaviors will in future recommender system be
inevitable.
