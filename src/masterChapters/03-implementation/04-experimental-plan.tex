...
information can then e.g. be used in a combination with a classification accuracy metric
to determine the recommendation list.
...
lead us to delve deeper into classification and ranking accuracy metrics.

%TODO - Add some more cites

The area under curve (AUC) is a popular classification accuracy metric and can be used as a single
measure for the overall quality of a recommender system. However, a frequently uttered point of criticism is
that users are often more interested in the items at the top of a recommendation list but that the AUC
measure is equally affected by swaps at the top or the bottom. To \emph{complement} AUC, we chose to
also measure the rank accuracy.
...
MAP vs. nDCG, rank distance, ERR, Expected Discounted Rank Correlation.
Also see: Computing distances between partial rankings

\marginpar{TODO: Discussion on suitedness/weaknesses of different rank accuracy metrics}
