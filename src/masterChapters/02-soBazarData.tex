...
\section{Dataset Overview}
    ...
    All
    When a user accesses a store or an item, the information regarding the user and the item is stored.
    Events such as \emph{purchasing} an item and \emph{wanting} an item is stored in the same manner.
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{l l}
            ...
            session           & Which session number the event belongs too~\tablefootnote{This is the value added in the preprocessing phase~\ref{sec:preprocessing}. For two events to end up in the same session, the event has to be triggered within a certain period of time, and both be after the same application started-flag} \\
...
\subsection{User properties}
    ...
    \end{figure}
        Figure~\ref{figure:eventIDDistribution} displays the count for each of the different events which can be triggered in the SoBazaar application.
        As seen from the graph the \emph{product\_detail\_clicked}-event and the \emph{storefront\_clicked}-event are the two most common events to be triggered, which is natural, since most of the items are located in the stores fronts.
    ...
    \end{figure}
        ...
        There are over 6000 products in the database, when 78\% of the users have less than 50 events, and as seen from figure~\ref{figure:eventIDDistribution}, the majority of events triggered by users are not directly item-related events, the information about items based on user actions alone will be sparse.
    ...
    \end{figure}
        ...
        The majority of the users of SoBazaar has a session count of 10 and less.
    ...
    \end{figure}
        ...
        As seen from figure~\ref{figure:ratingsPerUserCum}, over 20\% of the users have never interacted with an item, and about 10\% of the users have more than 40 item interactions.
    ...
    \end{figure}
        ...
        \marginpar{fact check that they use GPS and not IP-location, 6 decimals in data 11.575452}
...
\subsection{Time properties}
    ...
    \begin{figure}[H]
        \includegraphics[width=5in]{image/avglifetimeoncountdistribution.png}
        \centering
