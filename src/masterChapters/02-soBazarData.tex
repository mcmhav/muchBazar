...
\clearpage
  ...
  \label{sec:sobazaar-data}
	...
	SoBazaar is an online marketplace for fashion, but with a social twist. You can find stores like Moods of Norway,
	BIK BOK, Bianco and many more, which all have their own storefront in the application. The application lets you browse
	through thousands of products. If you find something you like you can purchase it directly from the application or store it
	for later with the \emph{lov it} function. The following screenshots are taken from the application and highlights some functionality
	currently found in the application. On the far left you see a newsfeed item from the \emph{homepage} or newsfeed. The newsfeed is
	currently shows what other users have liked, notifies the user about sales and new products in addition to recommending the most
	popular products.
	...
	The application brand screen shows on overview of the brands and storefront currently included in the application. The brand
	screen also features an editors pick page among other things. If you find a brand or store you like you can choose to enter their
	store and view their products.
	...
	From the product screen you can choose to buy items, by being redirected to the stores online webshop. The product screen
	currently features a \emph{People who like this also love} widget, and also shows other products from the same store. In addition you
	also have the possibility of finding stores close to you which sells the item.
	The application also features your own personal page for your saved or \emph{loved} items. In additional the application
...
\subsection{User properties}
    ...
    \end{figure}
        ...
        % todo
            % meant to introduce the reader to the idea that recommendations might be plausible
...
\subsection{Time properties}
    ...
    \end{figure}
        ...
        As we can see from the regression line, the average life span of an item increases with the count of item interactions, which is somewhat expected since they are in many ways intertwined, but we can also see that the regression line is not as steep as the
