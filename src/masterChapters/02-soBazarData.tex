...
\subsection{User properties}
    ...
    \end{figure}
        ...
        As seen from the graph the "product\_detail\_clicked"-event and the "storefront\_clicked"-event are the two most common events to be triggered, which is natural, since most of the items are located in the stores fronts.
        ...
        The "product\_wanted"-event is the fourth most occurring event, and is about half the size of the "product\_detail\_clicked"-event, this does not mean that half of the accessed items are added to a want list, since it is possible to want an item without accessing the item.
        "product\_purchase\_intended" on the other hand has to come after a "product\_detail\_clicked", about 8\% of item accesses progressed therefore into an intended purchase.
