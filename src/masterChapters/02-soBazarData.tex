...
\emph{heart it} function, which in this thesis is referred by its backend name
...
user-behaviors which might skew the learned stereotypical patterns and rendering them almost useless.
...
As part of the preprocessing, a unique \textit{session identifier} field was
...
will of course have no purchases. Making personalized recommendations for
these users unattainable.
...
order to make predictions. We do this by primarily looking at their
...
a trigger count lower than 200, which are merged into one bar and labeled
...
\emph{storefront} is the main access point for the user to get to a set of products, and the high
...
and without accessing the actual item -- that is sending a
...
item before sending the \textit{product\_purchase\_indtended} event\footnote{This was true during the time lifetime of the captured dataset. In later versions, the user has been given the option to press \emph{buy} without accessing the item} -- thus we
...
confirms this by presenting the cumulative graph where, 78\% of the users have
50 events or less, with an average of about 30 events per user. Note that we here look
...
the following key characteristics are apparent:

\begin{itemize}
  \item 90\% of all users has 60 or fewer item interactions, this is set as
  cut-off point in both figures.
  \item 70\% percent of the users has 17 or fewer item interactions.
...
accessing, to see if there is a usage pattern of some kind.
...
The users are group together, where one bucket represents the count of users
...
As we can see from the figure above the users are centered around $300$ NOK, where
$300$ NOK is the most common interaction price~\footnote{The median interaction
price is $299$ NOK}.

This tells us that the preferred price of the users in the SoBazaar application
is around $300$ NOK, which can be utilized when making ratings based on the user
events.
...
calculation further, approximately 50\% of the users has 6 sessions or fewer, all
...
city-centers, where most of the activity are coming from.
...
gain any valuable knowledge which might lead us to better understand the
...
Can we, however, make the same differentiation based on popularity? We have
...
investigate if we have a \emph{long-tail} by observing the following values.
...
methodology on products we can see which items that regularly are interacted
...
The figure above averaged the lifespan of the items and presented them
together with their respective count, below we show the same tendency, but keep
the lifespan separate.
...
perform a session study, looking at timing related behaviors in order to e.g.
...
scenario respectively. $e_b$ and $e_v$ are equally the standard errors for the
two scenarios. Using standard normal deviate (normally distributed random
...
As seen from the example above, first the user logs inn, which is the trigger flag to indicate a sessions is started. Then the user does a set of actions in the application.
...
\begin{figure}[H]
  ...
  \caption[Session states and their interactions]{A minimized view of the different states of the system and how they interact with each other.}
...
For a more complete view without reduced states see Figure~\ref{figure:statesInteractions}.
...
about the user preferences, other ways of gathering the preferences must be utilized.
