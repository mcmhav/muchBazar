...
\subsection{Product properties}
    ...
    \end{figure}
        ...
        This means that there will not be more than 10 events for the majority of the items, with no more than 10 datapoints to connect with other items for the majority of the items,
        Making recommendations through itembased recommendations might prove problematic.

        6000 items
            10 < evs on 80\% of items
            one item 10 or less possible connection points for 80\% of the items


        which might lead to an issue when doing collaborative filtering on the data.

        prove problematic -> itembased paer per germener explain
