...
defferentiators between users and their interactions on items, in order to
induce preference from implicit feedback. In our second component, we extend
this approach by further taking into account content features we strengthen our
ratings, a process which we call \textit{boosting} of ratings. We then, in our
third component, provide valuable insight and experiments using implicit
ratings in recommender system algorithms, spanning from taking simple
global-averages to State of the Art matrix factorization -- ultimatly yieding a
...
earlier components, seeking to identify both optimal methods and parameters.
...
items, by several factors. We find that by factoring in feature-trends for each
user, we achieve a set of generated ratings that combined, generally
outperforms all other feature alone. These promising results, coupled with a
robust, modular and highly adaptive system for making recommendations from
implicit feedback promotes further research into the area.
\end{abstract}
...
rolle i både kommersielle og akademiske miljø. Den vanligste framgangsmåten for
...
gode anbefalinger. I denne avhandlingen, ser vi istedet på hvordan disse
karakterene kan bli estimert ved å se brukerenes bruksmønstre og hvilke
produkter de viser interesse for.

Et system som bruker denne type estimering og som baserer sine estimeringer ved
hjelp av et ekstremt lite datasett, blir i denne avhandlingen foreslått.
Systemet som foreslås har fire hovedkomponenter som bygger på hverandre -- hvor
inndata er \textit{implisitt data} (brukermønstre/systemlogger) og anbefalinger
med tilhørende evalueringsresultat er utdataen. Første komponenten består av å
konvertere den implisitte dataen til estimerte karakterer, hvor det påvises at
pris, popularitet og tid er viktige produktfaktorerer for å bestemme potensiell
...
kompleksitetsgrad presenterer vi flere anbefalingssresultat som vi i siste
komponent gjør en evaluering av. I denne fjerde komponenten prøver vi å finne
optimale metoder både for å konvertere implisitt data, gjøre boosting og
anbefale produkter basert på implisitte karakterer. I tilleg har samtlige
metoder flere parametere, hvor vi analyserer hvilke som passer seg for vårt
domene og system.

Et lovene resultat blir til slutt presentert, hvor det foreslåtte systemet gir
bedre resultater enn alle andre testede anbefalingsmetoder, hvor kun antall
kjøp blir vurdert, inkludert scenariotet ved å kun anbefale de mest populære
...
avhandlingen -- kombinert med et modulært, robust og tilpasningsdyktig system
