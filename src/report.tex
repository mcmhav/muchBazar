...
Telenor and our supervisors at NTNU have been invaluable, every week.
...
recommender systems has gained an important role in both a commercial and
...
asking the users for a numeric rating for every item they consume. However,
this procedure requires important changes to the user interface and also an
active effort from all users. In this thesis we instead look at how these
ratings can be determined implicitly from looking at session data and
the transaction history for every user.

Using these generated implicit ratings we analyze which recommender models are
suitable for such an untraditional data source, whilst accounting for a sparse
dataset with few events – often called a cold start environment. Finally, a
discussion is made on the various evaluation metrics for both implicit ratings
and our recommender models, as here as well the untraditional approach of
choosing implicit ratings as input to our models makes many of the conventional
metrics inefficacious.

Multiple novel ways of generating ratings are presented taking into account
recentness in the data as an important factor. This, combined with
content-based approaches makes for a new and interesting way of solving the
cold start user-problem. We then provide valuable insight into using implicit
ratings in both traditional and untraditional recommender methods spanning from
simple global-averages to state-of-the-art matrix-factorizations. A promising
result is finally made, supported by a range of relevant metrics, promoting
further research into the area of implicit ratings on sparse and small datasets.
...
\input{masterChapters/02-stateOfTheArt}
\input{masterChapters/03-soBazarData}
\input{masterChapters/04-to-use} %todo name
\input{masterChapters/05-evaluation}
\input{masterChapters/06-implementation}
\input{masterChapters/07-result-evaluation}
\input{masterChapters/08-conclusion}
