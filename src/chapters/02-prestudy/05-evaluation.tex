...
%TODO - Write about bootstrapping?
...
\subparagraph{Mean Percentage Ranking (MPR)} % (fold)
...
% subparagraph mean_percentage_ranking_ (end)


\subparagraph{Mean Average Precision (MAP)} % (fold)
...
% subparagraph mean_average_precision_map_ (end)


\subparagraph{Normalized Discounted Cumulative Gain (nDCG)} % (fold)
...
% subparagraph normalized_discounted_cumulative_gain_ (end)

\subparagraph{Limitations} % (fold)
\label{subp:limitations}
As mentioned by Powers et al.~\cite{powers2007}, Recall, Precision, F-Measure and Rand Accuracy have Bias.asdfsdfsdf

% subparagraph limitations (end)
...
\subparagraph{AP correlation} % (fold)
...
% subparagraph ap_correlation (end)
