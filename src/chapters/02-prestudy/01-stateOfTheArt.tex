...
This section will present and discuss previous work in the field of recommender systems, existing solutions to the cold-start problem, fashion recommender systems and contain a discussion of the domain specific challenges of designing a recommender system for the fashion domain.
...
>>>>>>> 85cbcc7687e25c2ec438e91a02ede8f0571752fb
...
To what degree this increased interest can be mapped to a more substant \marginpar{checkout substance/ (substanse, mer fast/solid/handfast)} feedback varies from customer to customer.
...
OCCF can be viewed as an extreme case of class imbalance, in which the balance is only distributed over one class. \marginpar{checkout Extreme re-balancing for svms: a case study maybe}
...
=======
The issue with this approach is that putting the rating burden onto the user has shown to produce negative user experience. \marginpar{reference}
...
More specifically, given two matrices, one rating matrix $R = SxC$ \marginpar{0 or 1} where $S$ is a list of $m$ items, $S = {s_{1},...s_{m}}$ and $C$ is a list of $n$ users, $C = {c_{1}, ... c_{n}}$ and a corresponding weight matrix $W$\marginpar{between 0 and 1} where $S$ is a list of $m$ items, $S = {s_{1},...s_{m}}$ and $CWz$ is a list of $n$ users, $C = {c_{1}, ... c_{n}}$.
With these two matrices, weighted low rank approximation is meant to find a low rank matrix $X$, which minimizes the objective of a weighted Frobenius loss function. \marginpar{reread} \marginpar{cite}
>>>>>>> 85cbcc7687e25c2ec438e91a02ede8f0571752fb
...
=======
Equation~\ref{equation:frobenius} can be optimized and solved efficiently though the use of wighted ALS~\marginpar{cite}.
>>>>>>> 85cbcc7687e25c2ec438e91a02ede8f0571752fb
...
=======
\marginpar{fixequations}
\marginpar{maybe explain the schemes better}
\marginpar{add cite}
...
This subset is based on a sampling probability matrix $P$.  \marginpar{cite}
...
\paragraph{Em algorithms to predict negative examples}
\marginpar{check out the approaches}

PEBL: positive example based learning for web page classification using SVM
Building text classifiers using positive and unlabeled examples
Presence-only data and the EM algorithm \marginpar{ref}.


\paragraph{Low-density non-negative matrix factorization (ldNMF)}
arg min .~\marginpar{cite}
...
+
(i,j)∈U
\marginpar{make equation}
Equation.
$R = SxC$ \marginpar{0 or 1} is the rating matrix, where $S$ is a list of $m$ items, $S = {s_{1},...s_{m}}$ and $C$ is a list of $n$ users, $C = {c_{1}, ... c_{n}}$.
$L$ is the set of positive ratings, while $U$ is the set of unlabeled rating.
$W$ and $H$ are the latent factors.
$V$ is a loss function (squared loss or generalized KL-divergence).
$C$ is the entry specific costs.
...
\paragraph{Group Bayesian personalized ranking (GBPR)}

Utilizing rich user interaction.
Group preference is the preference score of a group of user on an item. \marginpar{equation}
GBPR assumes that the group preference is stronger than the observed user preference. \marginpar{cite}
GBPR =
tr j∈I tr I tr
u∈U tr i∈I u
u
